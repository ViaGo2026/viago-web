...
<h1>Trouble & Resolution</h1>

### 🛠️ 멀티 리전 WAF 오탐(False Positive) 레이턴시 및 이미지 업로드 차단 이슈 해결

- **문제 상황 (Issue)**
    - 사용자가 여행 후기에 작성한 특정 코드나 특수문자가 포함된 텍스트/이미지를 업로드할 때, 백엔드에서 AI 분석을 수행하는 과정 중 **AWS WAF**가 이를 XSS(Cross-Site Scripting) 또는 악성 주입(Injection) 공격 코드로 오인하여 **멀티 리전(Seoul & São Paulo) 전체에서 정상적인 이미지 업로드를 간헐적으로 차단(403 Forbidden)**하는 오탐 현상이 발생함.
- **원인 분석 (Root Cause)**
    - AWS WAF의 Managed Rule Set(관리형 규칙 그룹)이 엄격하게 적용되어 있어, AI 데이터 분석 파이프라인 내부의 정상적인 바이너리/스트링 데이터 패턴을 웹 취약점 공격으로 잘못 탐지함.
- **해결 방안 (Resolution)**
    - 전역 보안 규칙을 전면 무력화하는 대신, 특정 위험 경로에만 예외를 두는 **Scope-down Statement(검사 범위 축소 조건)**를 양대 리전(Seoul, São Paulo) WAF 웹 ACL 정책에 세밀하게 반영함.
    - **세부 설정:** `URI Path`가 이미지 업로드 전용 엔드포인트(예: `/api/v1/upload/*`)로 일치하는 요청에 대해서만 해당 Injection 방어 규칙의 검사 대상에서 제외(Exclude)하도록 매칭 조건을 격리함.
    - **결과:** 전체 인프라의 다층 웹 보안 프레임워크는 그대로 유지하면서, AI 검열을 위한 미디어 파일 업로드 경로의 오탐 이슈를 원천 해결하여 데이터 파이프라인의 가동률을 100%로 회복함.
 
  ### 🛠️ 인프라 공유 스토리지 장애 해결 및 관리 구조 개선

- **문제 상황 (Issue)**
    - OpenVPN 서버용 EC2 인스턴스에서 **NFS(Network File System)**를 활용하여 구성 파일 및 정적 자원을 동기화하는 과정에서 동시성 문제 및 마운트 타임아웃 등 **간헐적인 공유 스토리지 동기화 오류**가 발생함.
- **1차 조치 및 한계 (Temporary Fix)**
    - 리소스 부족(메모리 및 네트워크 스루풋 제한)으로 인한 인스턴스 병목 현상으로 가설을 설정하고, 인스턴스 타입을 **`t3.micro`에서 `t3.small`로 스케일 업(Scale-up)** 조치함.
    - 인프라 사양 증설 이후에도 NFS 프로토콜 자체의 네트워크 오버헤드와 파일 잠금(File Locking) 이슈 등으로 인해 간헐적 오류가 지속됨을 확인함.
- **최종 해결 방안 (Root Cause Architecture Improvement)**
    - 네트워크 결합도가 높고 단일 장애점(SPOF) 위험이 있는 NFS 방식을 과감히 폐기함.
    - 인프라 형상 관리의 안정성을 높이기 위해 **GitHub 기반의 형상 관리 분산 아키텍처**로 리팩토링을 단행함.
    - 설정 파일 및 코드를 Git 레포지토리에서 중앙 관리하고, 변경 사항 발생 시 인스턴스가 독립적으로 Pull 받아 동용하는 방식으로 전환하여 **네트워크 스토리지 의존성을 완전히 제거(Decoupling)**하고 시스템의 가동률과 신뢰성을 극대화함.
